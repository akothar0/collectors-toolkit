...
- a public `card-images` storage bucket for collection, scanner, and grader photos
- a private `import-files` bucket for import screenshots and PDFs
- `collection_cards.front_image_url` as the derived cover image for collection list/grid/portfolio views
- `collection_card_images` as the ordered gallery source of truth when the migration is available

If you see a schema cache error for `collection_card_images`, prefer a separate query against `collection_cards` plus a fallback fetch for gallery rows instead of assuming the relationship is broken. The cache usually needs a refresh after the migration is applied to Supabase.
